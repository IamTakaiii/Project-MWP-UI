...
import {
  useWorklogHistory,
  formatTimeSpent,
} from "@/hooks/use-worklog-history";
...
export function MiniHistory({ className }: MiniHistoryProps) {
  ...
  return (
    <div
      className={cn(
        "bg-card backdrop-blur-xl border border-border rounded-2xl overflow-hidden flex flex-col",
        className,
      )}
    >
      {/* Header */}
      <div className="p-5 border-b border-border bg-black/20">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {format(dateRange.start, "d MMM", { locale: th })} -{" "}
          {format(dateRange.end, "d MMM yyyy", { locale: th })}
        </p>
      </div>
      ...
      <div className="p-4 border-t border-border bg-black/10 mt-auto">
        <Link to="/history" search={(prev) => ({ ...prev, date: undefined })}>
