...
export function useSSE(url: string, options: UseSSEOptions = {}) {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        ...
        console.log('[useSSE] Using credentials: include (cookies should be sent automatically)')
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new Error(`Unauthorized - Check your authentication credentials. ${errorMessage}`)
        } else if (response.status === 403) {
          throw new Error(`Forbidden - You do not have access to this resource. ${errorMessage}`)
      ...
      setEvents(prev => [...prev, createEvent('connected', { 
      ...
      while (true) {
        ...
        if (done) {
          ...
          setEvents(prev => [...prev, createEvent('disconnected', { message: 'Stream ended' })])
          break
        }
          ...
          if (eventData) {
            ...
            try {
              ...
              if (typeof parsedData === 'object' && parsedData !== null && 'type' in parsedData) {
                eventType = String((parsedData as Record<string, unknown>).type)
              }
            } catch {
              // Keep as string
            }

            setEvents(prev => [...prev, createEvent(eventType, parsedData, eventData)])
          }
        }
      }
    } catch (e) {
      ...
      setIsConnected(false)
      setConnectionTime(null)
      setEvents(prev => [...prev, createEvent('error', { message: errorMessage })])
    }
  }, [])
  ...
  const disconnect = useCallback(() => {
    if (abortControllerRef.current) {
      ...
      setEvents(prev => [...prev, createEvent('disconnected', { message: 'Disconnected by user' })])
    }
    setIsConnected(false)
    setConnectionTime(null)
  }, [])
