...
export class ApiClient {
  ...
  protected async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        const errorResult = result as { error?: string; details?: string; message?: string }
        const message = errorResult.details || errorResult.error || errorResult.message || `HTTP ${response.status}`
