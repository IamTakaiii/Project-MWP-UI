        ...
        (async () => {
          ...
          try {
            ...
            if (!pathMatch) {
              ...
              res.end(
                JSON.stringify({ error: "Invalid proxy path", url: urlPath }),
              );
              return;
            }
            ...
            console.log(
              `[SSE Proxy] Extracted - host: ${host}, path: ${path}, targetUrl: ${targetUrl}`,
            );

            console.log(
              `[SSE Proxy] Proxying ${req.method || "GET"} to: ${targetUrl}`,
            );
            console.log(`[SSE Proxy] Original URL: ${req.url}`);
            console.log(
              `[SSE Proxy] Request headers:`,
              Object.keys(req.headers),
            );

            // Log all custom headers that might contain cookies
            const allHeaders = Object.entries(req.headers);
            const customHeaders = allHeaders.filter(
              ([key]) =>
            ...
            console.log(
              `[SSE Proxy] Custom/auth headers found:`,
              customHeaders.map(([key]) => key),
            );
            ...
            for (const headerName of customCookieHeaders) {
              ...
              if (headerValue) {
                ...
                if (cookieValue) {
                  ...
                  console.log(
                    `[SSE Proxy] Found cookie in custom header: ${headerName} (length: ${cookieValue.length})`,
                  );
                  break;
                }
              }
            ...
            if (!cookieValue && req.headers.cookie) {
              ...
              if (cookieValue) {
                cookieSource = "cookie";
                console.log(
                  `[SSE Proxy] Using Cookie header from browser (length: ${cookieValue.length})`,
                );
                console.log(
                  `[SSE Proxy] Cookie header value (first 100 chars):`,
            ...
            if (!cookieValue) {
              console.log(
                `[SSE Proxy] No cookie found in custom headers or browser`,
              );
            } else {
            ...
            if (host.includes("ngrok")) {
              ...
              console.log(
                "[SSE Proxy] Added ngrok-skip-browser-warning header",
              );
            }
            ...
            if (cookieValue) {
              ...
              console.log(
                `[SSE Proxy] Cookie header set from ${cookieSource} and will be forwarded to target`,
              );
            }
            ...
            console.log(
              `[SSE Proxy] Cookie in headers:`,
              headers["Cookie"] ? "Yes (hidden in log)" : "No",
            );
            ...
            if (headers["Cookie"]) {
              console.log(
                `[SSE Proxy] Cookie header value (first 100 chars):`,
                String(headers["Cookie"]).substring(0, 100),
              );
            }
            ...
            console.log(
              `[SSE Proxy] Target response status: ${targetResponse.status}`,
            );
            ...
            // Log response status
            console.log(
              `[SSE Proxy] Response status: ${targetResponse.status} ${targetResponse.statusText}`,
            );
            console.log(
            ...
            if (!targetResponse.ok) {
              const errorText = await targetResponse
                .text()
                .catch(() => "Unknown error");
              console.error(
                `[SSE Proxy] Error ${targetResponse.status} from ${targetUrl}`,
              );
              console.error(
                `[SSE Proxy] Error response:`,
                ...
                try {
                  while (true) {
                    ...
                    if (!res.writableEnded && !res.destroyed) {
                      ...
                      console.log(
                        "[SSE Proxy] Response closed, cancelling reader",
                      );
                      reader.cancel();
                      break;
                    }
